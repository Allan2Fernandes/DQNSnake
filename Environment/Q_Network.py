...
class Q_Network(nn.Module):
    def __init__(self, device, observation_space_size, action_space_size):
        ...
        self.device = device
        self.hidden_layer1 = nn.Linear(in_features=observation_space_size, out_features=128, device=self.device)
        self.hidden_layer2 = nn.Linear(in_features=128, out_features=256, device=self.device)
        # self.hidden_layer3 = nn.Linear(in_features=256, out_features=512, device=self.device)
        # self.hidden_layer4 = nn.Linear(in_features=512, out_features=256, device=device)
        ...
        # self.hidden_layer_activation4 = nn.ReLU()
    ...
    def forward(self, state):
        ...
        # x = self.hidden_layer_activation4(x)
        # x = self.bn4(x)
