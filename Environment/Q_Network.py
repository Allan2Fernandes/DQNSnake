...
class Q_Network(nn.Module):
    def __init__(self, device, observation_space_size, action_space_size):
        ...
        self.device = device
        self.hidden_layer1 = nn.Linear(in_features=observation_space_size, out_features=64, device=self.device)
        self.hidden_layer2 = nn.Linear(in_features=64, out_features=128, device=self.device)
        self.hidden_layer3 = nn.Linear(in_features=128, out_features=64, device=self.device)
        self.final_layer = nn.Linear(in_features=64, out_features=action_space_size, device=self.device)
